...
# herramientas para construir RAG (¡ACTUALIZADAS A LANGCHAIN_CLASSIC!)
...
ARCHIVO_PREGUNTAS = "Escenarios.xlsx"
...
# prompt del sistema
system_prompt = (
    "Eres un asistente experto en turismo y medio ambiente de Andalucía."
    "Tu tarea principal es responder basándote en el contexto proporcionado (los documentos PDF y enlaces web)."
    "Pero si el usuario pregunta sobre la ubicación geográfica (ciudades cercanas, provincias, distancias)"
    "y esa información no se menciona en el texto, puedes usar tu conocimiento general sobre geografía de España"
    "para deducir la respuesta."
    "Para datos específicos del sendero (longitud, dificultad, normas) o sitios de interés, usa SOLO el contexto."
    "\n\n"
    "{context}"
)
...
try:
    ...
    for p in preguntas:
        for i in range(1, NUM_REPETICIONES + 1):
            ...
            t0 = time.time()
            try:
                result = rag_chain.invoke({"input": p})
                resp = result['answer']
