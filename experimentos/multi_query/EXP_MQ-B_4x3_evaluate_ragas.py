...
# ==========================================
def run_mode_2():
    archivo_salida = "resultados_ragas_MQ-B4x3.xlsx"
    ...
    # prompt para que el LLM genere 4 versiones de la pregunta
    multiquery_prompt = PromptTemplate(
        input_variables=["question"],
        template=(
            "Eres un asistente experto en turismo activo y senderismo en Andalucía. "
            "Tu tarea es generar 4 versiones diferentes de la pregunta del usuario "
            "para recuperar documentos relevantes de una base de datos vectorial sobre "
            "rutas de senderismo y espacios naturales de Andalucía. "
            "Al generar múltiples perspectivas, ayudas a superar las limitaciones "
            "de la búsqueda por similitud semántica. "
            "Proporciona EXACTAMENTE 4 preguntas alternativas, una por línea, "
            "sin numeración, sin explicaciones, sin texto adicional. "
            "Pregunta original: {question}"
        ),
    )

    # configuracion del retriever con k=3 por subquery
    base_retriever = vectorstore.as_retriever(search_kwargs={"k": 3})

    # MultiQueryRetriever genera las subqueries y deduplica automáticamente
