...
# ==========================================
def run_mode_2():
    archivo_salida = "resultados_ragas_RRR-GE.xlsx"
    ...
    # prompt para hacer la pregunta más concreta precisando geografía y parámetros
    rewrite_prompt_rrr = ChatPromptTemplate.from_messages([
        ("system",
        "Eres un asistente que precisa preguntas para mejorar la búsqueda en "
        "una base de datos de senderismo andaluz. Dada la pregunta original "
        "y el contexto recuperado, reescribe la pregunta haciéndola más "
        "concreta: identifica la zona geográfica (provincia, parque natural, "
        "sierra), los parámetros mencionados (dificultad, duración, longitud) "
        "y los explicita en la pregunta reformulada. Si la pregunta es general, "
        "mantén la generalidad pero usa nombres propios cuando el contexto los "
        "sugiera. Devuelve ÚNICAMENTE la pregunta reformulada."),
